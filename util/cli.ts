...
import { WIKI_TOKEN_KEY } from "./const";
...
async function writeToken(token, wikiToken) {
  let finalToken = token;
  let finalWikiToken = wikiToken;
  if (!token) {
    finalToken = await promptUser(
      `Enter your token, [Usage: npm run bot:setup <bot token> <wiki token>]`
    );
  }
  if(!wikiToken){
    finalWikiToken = await promptUser(
      `Enter your wiki token, [Usage: npm run bot:setup <bot token> <wiki token>]`
    );
  }
  // destructively overwrites .env as BOT_TOKEN=xxxxxxxx
  const envObject = { 
    [botTokenKey]: finalToken ,
    [WIKI_TOKEN_KEY]: finalWikiToken
  };
...
async function run(command, ...args) {
  switch (command) {
    case "reset":
      ...
      break;
    case "setup":
      await writeToken(args[0], args[1]);
      break;
    case "addsecret":
...
Usage:
  ...
  $ npm run bot:setup <token> <wiki token>
...
Commands:
  ...
  5. Setup Command:
     $ npm run bot:setup <token> <wiki token>
     Overwrite (delete all other values) in .env file with BOT_TOKEN=<token> and WIKI_TOKEN=<wiki token>
